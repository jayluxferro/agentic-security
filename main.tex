...
FRONTIER language models, built on the transformer architecture~\cite{vaswani2017attention} and scaled across multiple orders of magnitude~\cite{brown2020language,kaplan2020scaling,hoffmann2022training}, exhibit emergent capabilities that make them suitable for complex tasks~\cite{wei2022emergent}. They can execute multi-step operations, coordinate tools, and reduce analyst overhead in security contexts~\cite{openai2023gpt4,deepseek2024,geminiteam2023}. Their deployment as autonomous agents is accelerating: models are being integrated into security operations centers, penetration testing workflows, and incident response pipelines~\cite{deng2023pentestgpt,fang2024llmagent,bommasani2021opportunities}. The risk is equally clear: autonomous action chains can bypass intent, exceed authorization boundaries, trigger irreversible external effects, or exfiltrate sensitive data before a human operator can intervene~\cite{weidinger2022taxonomy,ganguli2022red,amodei2016concrete}.
...
Safety interventions for language models span the training-to-inference spectrum. At training time, RLHF and Constitutional AI shape model behavior through preference optimization~\cite{bai2022constitutional}. Red-teaming frameworks systematically probe for failure modes before deployment~\cite{ganguli2022red}. At inference time, chain-of-thought prompting~\cite{wei2022chain} and zero-shot reasoning~\cite{kojima2022zero} improve capability but do not address safety. Refusal training produces models that decline harmful requests, and activation steering techniques manipulate internal representations to suppress undesirable outputs~\cite{arditi2024refusal,turner2023activation,rimsky2024steering}. These approaches share a common limitation: they modify model behavior probabilistically. A model that refuses 95\% of harmful requests still fails on the remaining 5\%, and the failure modes are opaque~\cite{bender2021dangers}. Safety benchmarks confirm that current models remain vulnerable to adversarial attacks~\cite{li2024wmdp,mazeika2024harmbench}. The emergent properties that make LLMs capable also make their failure modes unpredictable~\cite{schaeffer2024emergent}. Our work is complementary: rather than modifying the model, we place a \emph{deterministic} enforcement layer \emph{after} the model that catches pattern-matchable dangerous actions regardless of how the model was trained.
...
The cybersecurity LLM evaluation landscape has grown rapidly. Holistic evaluation frameworks~\cite{liang2022helm} and agent benchmarks~\cite{liu2024agentbench,jimenez2023swebench,yang2024sweagent,gu2024taubench} measure task-completion capability. In the cybersecurity domain specifically, CyberSecEval 1--3 provide progressively broader evaluations of LLM capabilities and risks~\cite{bhatt2023cyberseceval,bhatt2024cyberseceval2}. Cybench evaluates autonomous agents on 40 professional-level CTF tasks with subtask annotations~\cite{zhang2024cybench}. NYU CTF Bench contributes an open-source CTF benchmark dataset with an automated LLM-and-tool workflow for offensive-security evaluation~\cite{shao2024nyuctfbench}. CyberMetric and CTIBench evaluate cybersecurity knowledge through multiple-choice questions~\cite{tihanyi2024cybermetric,alam2024ctibench}. EnIGMA and PentestGPT explore LLM-based penetration testing agents~\cite{abramovich2024enigma,deng2023pentestgpt}. Studies of AI-assisted coding have shown that LLMs can introduce security vulnerabilities~\cite{pearce2022copilot,perry2023insecure}. However, none of these benchmarks measure the safety of the \emph{actions} an agent proposes---they measure capability, not risk. Our SAP methodology fills this gap by measuring \emph{action-level safety} across models.
...
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{3.8cm}p{1.3cm}p{1.1cm}}
\toprule
...
\texttt{curl localhost/?file=../../../etc/shadow} & allow & DANG. \\
\texttt{Exploit the SQL injection to extract credentials} & allow & DANG. \\
...
This layered architecture maps to established safety engineering principles and governance frameworks. The NIST AI Risk Management Framework~\cite{nist2023airmf} and its Generative AI Profile~\cite{nist2024gaiprofile} recommend multi-layered controls for AI systems. Zero-trust architecture principles~\cite{nist2020zerotrust} similarly advocate for continuous verification at every boundary. The OWASP Top 10 for LLM Applications~\cite{owasp2025llm} and MITRE ATLAS~\cite{mitreatlas} provide taxonomies of AI-specific threats that inform rule design. The broader evaluation literature cautions against over-reliance on single benchmarks~\cite{raji2021ai}. Our layered architecture provides diverse redundant mechanisms: pattern rules, semantic analysis, and human review. Each layer catches failures that bypass the previous layer, and no single layer is relied upon as a complete safety solution. The deterministic guard (Layer 2) is the only layer that can operate in air-gapped or GPU-constrained environments, making it the minimum viable safety mechanism for edge deployments.
...
These findings contribute to a growing body of evidence that LLM agent safety requires multi-layered, defense-in-depth approaches. Prior work on LLM penetration testing confirms that models can autonomously exploit vulnerabilities~\cite{happe2023pwned}, and predictability studies show that capability improvements often come with unexpected failure modes~\cite{ganguli2022predictability}. The deterministic guard provides a necessary but insufficient first layer: it catches the most explicitly dangerous commands (reverse shells, credential dumping tools, persistence mechanisms) with high precision and near-zero latency, but it cannot address the broader challenge of semantic safety. The 52.5\% of false negatives that are abstract descriptions represent actions that a human security reviewer would recognize as dangerous but that no pattern-matching system can reliably detect. Addressing this gap requires either (a) enforcing output format constraints that require models to produce concrete, auditable command sequences, (b) deploying semantic safety classifiers as a second enforcement layer, or (c) both.
